...
(async()=>{
 ...
 try{
  ...
  let role='MANAGER',prepared=false,done=false,loadFail=true,countLost=true,applyLost=true;
  ...
  await login('rooms');await page.getByRole('button',{name:'Минибарын тохиргоо',exact:true}).click();await page.getByLabel('Тоолох цэвэрлэгч').selectOption('cleaner');await page.getByRole('button',{name:'Тооллого даалгах',exact:true}).click();await ready();assert(prepared);
  role='CLEANER';await login('cleaning');await page.getByRole('button',{name:'Минибарын тохиргооны ажлууд',exact:true}).click();await page.getByRole('button',{name:'Тохиргооны ажлыг дахин ачаалах',exact:true}).click();
  ...
  fs.mkdirSync('artifacts',{recursive:true});await page.screenshot({path:'artifacts/minibar-reconciliation-desktop.png',fullPage:true});await page.setViewportSize({width:320,height:760});await page.emulateMedia({reducedMotion:'reduce'});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await page.screenshot({path:'artifacts/minibar-reconciliation-mobile.png',fullPage:true});
  ...
  assert.equal(await page.evaluate(()=>localStorage.length+sessionStorage.length),0);assert.deepEqual(errors,[]);fs.writeFileSync('artifacts/minibar-reconciliation-requests.json',JSON.stringify(requests));
  console.log('Minibar reconciliation browser: assignment, physical counts, atomic apply, lost-response retries, validation, load recovery and mobile passed.');
