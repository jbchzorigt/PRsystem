...
(async()=>{
 ...
 try{
  ...
  let role='MANAGER',revision=1,financeRevision=3,corrected=false,conflict=true,failRead=true;
  ...
  await page.route('**/hotels/**',async r=>{
   ...
   }else if(tail==='stays/stay/refunds'){assert.equal(role,'RECEPTION');assert.equal(body.receipt_id,'payment');assert.equal(body.amount_mnt,3000);result={refund_id:'refund',state:'RESERVED'};}
