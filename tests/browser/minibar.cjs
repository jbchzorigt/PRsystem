...
(async()=>{
  ...
  try{
    ...
    await send.click();await submittedForm.evaluate(f=>f.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true})));
    ...
    await submittedForm.evaluate(f=>{if(f.isConnected)throw Error('Expected completed form to be detached');f.dispatchEvent(new Event('submit',{bubbles:true,cancelable:true}));});await submittedForm.dispose();
