...
(async()=>{
 ...
 try{
  ...
  let requests=[],failure=false,created=false,paid=false,closed=false,mode='RECEPTION';const problems=[];page.on('pageerror',e=>problems.push(e.message));
  ...
  await page.route('**/hotels/**',async route=>{
   ...
   else if(tail==='stays/active')data=created&&!closed?[stay]:[];else if(tail==='bookings'||tail==='handovers'||tail==='cleaning/checkouts')data=[];
   ...
   else if(tail==='stays/stay-1/finance')data={balance:{revision:paid?3:1,available:paid?0:60000,refund_reserved:0,frozen:false},charge_unpaid_mnt:paid?0:80000,charges:[{id:'charge-1',kind:'ROOM',amount_mnt:80000,paid_mnt:paid?80000:0}],receipts:paid?[]:[{id:'receipt-1',purpose:'DEPOSIT',channel:'CASH',amount_mnt:60000,allocated:0,refund_reserved:0,refunded:0,reversed:0}],refunds:[],corrections:[],payment_intents:[]};
   else if(tail==='stays/stay-1/checkout/preview')data={inspection:null,report:null,restaurant_orders:[]};
   else if(tail==='stays/stay-1/cash-receipts'){assert.equal(body.expected_revision,1);assert.equal(body.amount_mnt,80000);assert.equal(body.received,true);paid=true;data={state:'COMPLETED'};}
  ...
  await page.getByRole('button',{name:'Байрлалтыг нээх',exact:true}).click();await page.getByRole('button',{name:'Төлбөр авах',exact:true}).click();await page.getByRole('button',{name:'Дүн оруулах',exact:true}).click();await page.getByLabel('Бэлэн мөнгийг биечлэн авсан',{exact:true}).check();await page.getByRole('button',{name:'Төлбөр бүртгэх',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('.result')).some(n=>n.textContent.includes('Дууссан')));await page.getByRole('button',{name:'Байрлалт шинэчлэх',exact:true}).click();await page.getByRole('button',{name:'Зочны checkout баталгаажуулах',exact:true}).click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('.result')).some(n=>n.textContent.includes('Хаасан')));
