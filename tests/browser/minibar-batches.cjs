...
(async()=>{
 ...
 try{
  ...
  const batch=(id='b1')=>{const retry=id==='b2',items=retry?[{room_id:'r1',room_number:'101',state:'READY_FOR_RECONCILIATION',revision:1}]:[{room_id:'r0',room_number:'100',state:'APPLIED',revision:2},{room_id:'r1',room_number:'101',state:cancelled?'CANCELLED':'READY_FOR_RECONCILIATION',revision:cancelled?2:1},{room_id:'r100',room_number:'200',state:'SKIPPED',revision:0,code:'ROLLOUT_UNCHANGED'}];return{batch_id:id,target:{version_number:2},items,revision:(cancelled?'b':'a').repeat(64),state:cancelled&&!retry?'PARTIALLY_COMPLETED':'IN_PROGRESS',counts:{READY_FOR_RECONCILIATION:items.filter(i=>i.state==='READY_FOR_RECONCILIATION').length,selected:items.length,accepted:retry?1:2,APPLIED:retry?0:1,SKIPPED:retry?0:1,CANCELLED:cancelled&&!retry?1:0},reason:'Бүлгээр шинэчлэх',recorded_at:'2026-09-10T00:00:00Z',retry_of_batch_id:retry?'b1':null};};
  ...
  await check('100').waitFor();await page.getByRole('button',{name:'Энэ хэсгийг сонгох',exact:true}).click();await page.getByText('Сонгосон: 100 / 100',{exact:true}).waitFor();await page.getByRole('button',{name:'Сонголтын дараагийн хэсэг'}).click();await check('200').waitFor();assert(await check('200').isDisabled());await page.getByRole('button',{name:'Бүх сонголтыг арилгах',exact:true}).click();assert(await check('200').isEnabled());await page.getByRole('button',{name:'Сонголтын эхний хэсэг'}).click();
  ...
  await submit();await page.locator('.result.error').waitFor();await submit();await page.getByRole('heading',{name:'Багцын үлдсэн ажлыг цуцлах',exact:true}).waitFor();await ready();await page.getByRole('table',{name:'Төлөв бүрийн өрөөний тоо',exact:true}).waitFor();assert.equal(await page.getByRole('table',{name:'Төлөв бүрийн өрөөний тоо',exact:true}).locator('tbody tr').count(),10);assert.equal(await page.getByRole('table',{name:'Төлөв бүрийн өрөөний тоо',exact:true}).locator('tbody tr').first().locator('td').last().textContent(),'1');
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await page.screenshot({path:'artifacts/minibar-batch-progress-mobile.png',fullPage:true});await page.setViewportSize({width:1280,height:900});await page.screenshot({path:'artifacts/minibar-batch-progress-desktop.png',fullPage:true});await page.setViewportSize({width:320,height:760});
