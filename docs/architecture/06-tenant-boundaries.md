...
Four independent layers. Any one failing must not create an exposure.
...
| L4 — Test | Cross-tenant probe suite over every read and write endpoint | Regressions |
...
Open question **DM-01**. Mandatory predicates plus composite keys are the baseline. Postgres RLS is
attractive as a fifth layer but interacts with connection pooling and migration roles. The decision is
recorded in Phase 03 as an ADR; it is an addition to L2/L3, never a replacement.
