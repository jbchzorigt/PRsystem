# Phase 15 governed battery — execution log

The per-command record of the governed battery measured for the Phase 15 evidence, kept in the
repository so the manifest and the evidence table in [phase-status.md](phase-status.md#phase-15-record)
...
- **Tree:** a detached `git worktree` of `ff121ef641ac36d5087ec9cd3823bbe8056e71c3`, the Phase 15 implementation
...
- **Preparation, not evidence:** a single `pnpm run build` before the first command, as a CI job's
  ...
  workspace packages from it (exit 0, 19s). Every gate below ran after it and rebuilt under force.
...
- **Timing:** commit ff121ef641ac36d5087ec9cd3823bbe8056e71c3 started 2026-09-06T10:18:37Z; finished 2026-09-06T10:30:58Z.
...
| `node tools/validate-governance.fixtures.mjs` | 1 | 0 | 32s |
...
| `pnpm run test:concurrency` | 1 | 0 | 23s |
| `pnpm run test:concurrency` | 2 | 0 | 26s |
| `pnpm run test:concurrency` | 3 | 0 | 23s |
| `pnpm run test:regression` | 1 | 0 | 25s |
| `pnpm run test:security` | 1 | 0 | 136s |
| `pnpm run test:security` | 2 | 0 | 142s |
| `pnpm run test:security` | 3 | 0 | 133s |
...
[phase-15-evidence.json](phase-15-evidence.json) and restated in the evidence table of the Phase 13
