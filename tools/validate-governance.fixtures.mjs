...
const PHASE11_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-11-evidence.json');
const PHASE12_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-12-evidence.json');
const PHASE13_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-13-evidence.json');
const PHASE14_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-14-evidence.json');
const PHASE15_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-15-evidence.json');
const PHASE16_MANIFEST = join(ROOT, 'docs', 'implementation', 'phase-16-evidence.json');
...
const originalPhase11Manifest = readFileSync(PHASE11_MANIFEST, 'utf8');
const originalPhase12Manifest = readFileSync(PHASE12_MANIFEST, 'utf8');
const originalPhase13Manifest = readFileSync(PHASE13_MANIFEST, 'utf8');
const originalPhase14Manifest = readFileSync(PHASE14_MANIFEST, 'utf8');
const originalPhase15Manifest = readFileSync(PHASE15_MANIFEST, 'utf8');
const originalPhase16Manifest = readFileSync(PHASE16_MANIFEST, 'utf8');
...
const SOURCES = {
  ...
  'phase08-manifest': {
    text: originalPhase08Manifest,
    env: 'PRSYSTEM_PHASE08_MANIFEST',
    file: 'phase-08-evidence.json',
  },
  'phase09-manifest': {
  ...
  'phase11-manifest': {
    text: originalPhase11Manifest,
    env: 'PRSYSTEM_PHASE11_MANIFEST',
    file: 'phase-11-evidence.json',
  },
  'phase12-manifest': {
    text: originalPhase12Manifest,
    env: 'PRSYSTEM_PHASE12_MANIFEST',
    file: 'phase-12-evidence.json',
  },
  'phase13-manifest': {
    text: originalPhase13Manifest,
    env: 'PRSYSTEM_PHASE13_MANIFEST',
    file: 'phase-13-evidence.json',
  },
  'phase14-manifest': {
    text: originalPhase14Manifest,
    env: 'PRSYSTEM_PHASE14_MANIFEST',
    file: 'phase-14-evidence.json',
  },
  'phase15-manifest': {
    text: originalPhase15Manifest,
    env: 'PRSYSTEM_PHASE15_MANIFEST',
    file: 'phase-15-evidence.json',
  },
  'phase16-manifest': {
    text: originalPhase16Manifest,
    env: 'PRSYSTEM_PHASE16_MANIFEST',
    file: 'phase-16-evidence.json',
  },
  'phase17-manifest': {
    text: originalPhase17Manifest,
    env: 'PRSYSTEM_PHASE17_MANIFEST',
    file: 'phase-17-evidence.json',
  },
  'phase18-manifest': {
    text: originalPhase18Manifest,
    env: 'PRSYSTEM_PHASE18_MANIFEST',
    file: 'phase-18-evidence.json',
  },
  'phase19-manifest': {
    text: originalPhase19Manifest,
  ...
  'phase21-manifest': {
    text: originalPhase21Manifest,
    env: 'PRSYSTEM_PHASE21_MANIFEST',
    file: 'phase-21-evidence.json',
  },
  'phase22-manifest': {
...
/** Applies `change` only inside the Phase 08 evidence region (check 17). */
function inPhase08Region(text, change) {
  const begin = text.indexOf('<!-- phase-08-evidence:begin -->');
  const endMarker = '<!-- phase-08-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 08 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
...
/** Applies `change` only inside the Phase 11 evidence region (check 17). */
function inPhase11Region(text, change) {
  const begin = text.indexOf('<!-- phase-11-evidence:begin -->');
  const endMarker = '<!-- phase-11-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 11 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 11 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 18 evidence region (check 17). */
function inPhase18Region(text, change) {
  const begin = text.indexOf('<!-- phase-18-evidence:begin -->');
  const endMarker = '<!-- phase-18-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 18 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 18 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 19 evidence region (check 17). */
function inPhase19Region(text, change) {
  const begin = text.indexOf('<!-- phase-19-evidence:begin -->');
  const endMarker = '<!-- phase-19-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 19 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
...
/** Applies `change` only inside the Phase 21 evidence region (check 17). */
function inPhase21Region(text, change) {
  const begin = text.indexOf('<!-- phase-21-evidence:begin -->');
  const endMarker = '<!-- phase-21-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 21 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 21 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 22 evidence region (check 17). */
function inPhase22Region(text, change) {
  const begin = text.indexOf('<!-- phase-22-evidence:begin -->');
  const endMarker = '<!-- phase-22-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 22 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
...
/** Applies `change` only inside the Phase 17 evidence region (check 17). */
function inPhase17Region(text, change) {
  const begin = text.indexOf('<!-- phase-17-evidence:begin -->');
  const endMarker = '<!-- phase-17-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 17 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 17 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 16 evidence region (check 17). */
function inPhase16Region(text, change) {
  const begin = text.indexOf('<!-- phase-16-evidence:begin -->');
  const endMarker = '<!-- phase-16-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 16 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 16 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 15 evidence region (check 17). */
function inPhase15Region(text, change) {
  const begin = text.indexOf('<!-- phase-15-evidence:begin -->');
  const endMarker = '<!-- phase-15-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 15 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 15 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 14 evidence region (check 17). */
function inPhase14Region(text, change) {
  const begin = text.indexOf('<!-- phase-14-evidence:begin -->');
  const endMarker = '<!-- phase-14-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 14 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 14 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 13 evidence region (check 17). */
function inPhase13Region(text, change) {
  const begin = text.indexOf('<!-- phase-13-evidence:begin -->');
  const endMarker = '<!-- phase-13-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 13 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 13 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 12 evidence region (check 17). */
function inPhase12Region(text, change) {
  const begin = text.indexOf('<!-- phase-12-evidence:begin -->');
  const endMarker = '<!-- phase-12-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 12 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 12 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}

/** Applies `change` only inside the Phase 09 evidence region (check 17). */
function inPhase09Region(text, change) {
  const begin = text.indexOf('<!-- phase-09-evidence:begin -->');
  const endMarker = '<!-- phase-09-evidence:end -->';
  const end = text.indexOf(endMarker);
  if (begin < 0 || end < 0) throw new Error('the Phase 09 evidence markers are missing');
  const region = text.slice(begin, end + endMarker.length);
  const changed = change(region);
  if (changed === region) throw new Error('the change did not alter the Phase 09 evidence region');
  return text.slice(0, begin) + changed + text.slice(end + endMarker.length);
}
...
const FIXTURES = [
  ...
  },
  {
    ...
    // manifest that wrote ACCEPTED would be declaring a customer decision the
    // governed state does not record.
    name: 'phase 08 manifest: an acceptance the governed state does not record',
    file: 'phase08-manifest',
    expect: /Phase 08 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 08 manifest: a non-zero exit recorded',
    file: 'phase08-manifest',
    expect: /records a non-zero exit code in the Phase 08 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    // Phase 08 evidence measured on the tree Phase 05 was accepted at would let
    // that acceptance stand in for a review of Phase 08.
    name: 'phase 08 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase08-manifest',
    expect: /Phase 08 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 08 manifest: a required command removed',
    file: 'phase08-manifest',
    expect: /Phase 08 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 08 manifest: the concurrency gate run once instead of three times',
    file: 'phase08-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 08 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 08 manifest: a key the contract does not declare',
    file: 'phase08-manifest',
    expect: /Phase 08 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 08 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-08-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-08-evidence:begin -->\n', '')
        .replace('<!-- phase-08-evidence:end -->\n', ''),
  },
  {
    name: 'phase 08 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 08 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase08Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 08 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 08 evidence was measured at/,
    mutate: (text) =>
      inPhase08Region(text, (region) =>
        region.replace(
  ...
  // ------------------------------------------------ Phase 09 evidence (check 17)
  {
    // The authorization of 2026-09-03 is implementation authorization only. A
    // manifest that wrote ACCEPTED would be declaring a customer decision the
    // governed state does not record.
    name: 'phase 09 manifest: an acceptance the governed state does not record',
    file: 'phase09-manifest',
    expect: /Phase 09 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 09 manifest: a non-zero exit recorded',
    file: 'phase09-manifest',
    expect: /records a non-zero exit code in the Phase 09 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    // Phase 09 evidence measured on the tree Phase 05 was accepted at would let
    // that acceptance stand in for a review of Phase 09.
    name: 'phase 09 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase09-manifest',
    expect: /Phase 09 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 09 manifest: a required command removed',
    file: 'phase09-manifest',
    expect: /Phase 09 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 09 manifest: the concurrency gate run once instead of three times',
    file: 'phase09-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 09 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 09 manifest: a key the contract does not declare',
    file: 'phase09-manifest',
    expect: /Phase 09 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 09 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-09-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-09-evidence:begin -->\n', '')
        .replace('<!-- phase-09-evidence:end -->\n', ''),
  },
  {
    name: 'phase 09 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 09 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase09Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 09 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 09 evidence was measured at/,
    mutate: (text) =>
      inPhase09Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 11 evidence (check 17)
  {
    name: 'phase 11 manifest: an acceptance the governed state does not record',
    file: 'phase11-manifest',
    expect: /Phase 11 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 11 manifest: a non-zero exit recorded',
    file: 'phase11-manifest',
    expect: /records a non-zero exit code in the Phase 11 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 11 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase11-manifest',
    expect: /Phase 11 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 11 manifest: a required command removed',
    file: 'phase11-manifest',
    expect: /Phase 11 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 11 manifest: the concurrency gate run once instead of three times',
    file: 'phase11-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 11 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 11 manifest: a key the contract does not declare',
    file: 'phase11-manifest',
    expect: /Phase 11 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 11 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-11-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-11-evidence:begin -->\n', '')
        .replace('<!-- phase-11-evidence:end -->\n', ''),
  },
  {
    name: 'phase 11 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 11 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase11Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 11 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 11 evidence was measured at/,
    mutate: (text) =>
      inPhase11Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 12 evidence (check 17)
  {
    name: 'phase 12 manifest: an acceptance the governed state does not record',
    file: 'phase12-manifest',
    expect: /Phase 12 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 12 manifest: a non-zero exit recorded',
    file: 'phase12-manifest',
    expect: /records a non-zero exit code in the Phase 12 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 12 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase12-manifest',
    expect: /Phase 12 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 12 manifest: a required command removed',
    file: 'phase12-manifest',
    expect: /Phase 12 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 12 manifest: the concurrency gate run once instead of three times',
    file: 'phase12-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 12 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 12 manifest: a key the contract does not declare',
    file: 'phase12-manifest',
    expect: /Phase 12 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 12 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-12-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-12-evidence:begin -->\n', '')
        .replace('<!-- phase-12-evidence:end -->\n', ''),
  },
  {
    name: 'phase 12 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 12 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase12Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 12 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 12 evidence was measured at/,
    mutate: (text) =>
      inPhase12Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 18 evidence (check 17)
  {
    name: 'phase 18 manifest: an acceptance the governed state does not record',
    file: 'phase18-manifest',
    expect: /Phase 18 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 18 manifest: a non-zero exit recorded',
    file: 'phase18-manifest',
    expect: /records a non-zero exit code in the Phase 18 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 18 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase18-manifest',
    expect: /Phase 18 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 18 manifest: a required command removed',
    file: 'phase18-manifest',
    expect: /Phase 18 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 18 manifest: the concurrency gate run once instead of three times',
    file: 'phase18-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 18 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 18 manifest: a key the contract does not declare',
    file: 'phase18-manifest',
    expect: /Phase 18 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 18 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-18-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-18-evidence:begin -->\n', '')
        .replace('<!-- phase-18-evidence:end -->\n', ''),
  },
  {
    name: 'phase 18 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 18 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase18Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 18 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 18 evidence was measured at/,
    mutate: (text) =>
      inPhase18Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 19 evidence (check 17)
  {
    name: 'phase 19 manifest: an acceptance the governed state does not record',
    file: 'phase19-manifest',
    expect: /Phase 19 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 19 manifest: a non-zero exit recorded',
    file: 'phase19-manifest',
    expect: /records a non-zero exit code in the Phase 19 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 19 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase19-manifest',
    expect: /Phase 19 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 19 manifest: a required command removed',
    file: 'phase19-manifest',
    expect: /Phase 19 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 19 manifest: the concurrency gate run once instead of three times',
    file: 'phase19-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 19 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 19 manifest: a key the contract does not declare',
    file: 'phase19-manifest',
    expect: /Phase 19 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 19 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-19-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-19-evidence:begin -->\n', '')
        .replace('<!-- phase-19-evidence:end -->\n', ''),
  },
  {
    name: 'phase 19 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 19 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase19Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 19 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 19 evidence was measured at/,
    mutate: (text) =>
      inPhase19Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 21 evidence (check 17)
  {
    name: 'phase 21 manifest: an acceptance the governed state does not record',
    file: 'phase21-manifest',
    expect: /Phase 21 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 21 manifest: a non-zero exit recorded',
    file: 'phase21-manifest',
    expect: /records a non-zero exit code in the Phase 21 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 21 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase21-manifest',
    expect: /Phase 21 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 21 manifest: a required command removed',
    file: 'phase21-manifest',
    expect: /Phase 21 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 21 manifest: the concurrency gate run once instead of three times',
    file: 'phase21-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 21 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 21 manifest: a key the contract does not declare',
    file: 'phase21-manifest',
    expect: /Phase 21 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 21 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-21-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-21-evidence:begin -->\n', '')
        .replace('<!-- phase-21-evidence:end -->\n', ''),
  },
  {
    name: 'phase 21 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 21 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase21Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 21 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 21 evidence was measured at/,
    mutate: (text) =>
      inPhase21Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 22 evidence (check 17)
  {
    name: 'phase 22 manifest: an acceptance the governed state does not record',
    file: 'phase22-manifest',
    expect: /Phase 22 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 22 manifest: a non-zero exit recorded',
    file: 'phase22-manifest',
    expect: /records a non-zero exit code in the Phase 22 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 22 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase22-manifest',
    expect: /Phase 22 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 22 manifest: a required command removed',
    file: 'phase22-manifest',
    expect: /Phase 22 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 22 manifest: the concurrency gate run once instead of three times',
    file: 'phase22-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 22 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 22 manifest: a key the contract does not declare',
    file: 'phase22-manifest',
    expect: /Phase 22 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 22 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-22-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-22-evidence:begin -->\n', '')
        .replace('<!-- phase-22-evidence:end -->\n', ''),
  },
  {
    name: 'phase 22 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 22 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase22Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 22 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 22 evidence was measured at/,
    mutate: (text) =>
      inPhase22Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  {
    name: 'phase 17 manifest: an acceptance the governed state does not record',
    file: 'phase17-manifest',
    expect: /Phase 17 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 17 manifest: a non-zero exit recorded',
    file: 'phase17-manifest',
    expect: /records a non-zero exit code in the Phase 17 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 17 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase17-manifest',
    expect: /Phase 17 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 17 manifest: a required command removed',
    file: 'phase17-manifest',
    expect: /Phase 17 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 17 manifest: the concurrency gate run once instead of three times',
    file: 'phase17-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 17 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 17 manifest: a key the contract does not declare',
    file: 'phase17-manifest',
    expect: /Phase 17 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 17 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-17-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-17-evidence:begin -->\n', '')
        .replace('<!-- phase-17-evidence:end -->\n', ''),
  },
  {
    name: 'phase 17 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 17 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase17Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 17 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 17 evidence was measured at/,
    mutate: (text) =>
      inPhase17Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 16 evidence (check 17)
  {
    name: 'phase 16 manifest: an acceptance the governed state does not record',
    file: 'phase16-manifest',
    expect: /Phase 16 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 16 manifest: a non-zero exit recorded',
    file: 'phase16-manifest',
    expect: /records a non-zero exit code in the Phase 16 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 16 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase16-manifest',
    expect: /Phase 16 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 16 manifest: a required command removed',
    file: 'phase16-manifest',
    expect: /Phase 16 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 16 manifest: the concurrency gate run once instead of three times',
    file: 'phase16-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 16 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 16 manifest: a key the contract does not declare',
    file: 'phase16-manifest',
    expect: /Phase 16 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 16 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-16-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-16-evidence:begin -->\n', '')
        .replace('<!-- phase-16-evidence:end -->\n', ''),
  },
  {
    name: 'phase 16 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 16 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase16Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 16 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 16 evidence was measured at/,
    mutate: (text) =>
      inPhase16Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 15 evidence (check 17)
  {
    name: 'phase 15 manifest: an acceptance the governed state does not record',
    file: 'phase15-manifest',
    expect: /Phase 15 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 15 manifest: a non-zero exit recorded',
    file: 'phase15-manifest',
    expect: /records a non-zero exit code in the Phase 15 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 15 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase15-manifest',
    expect: /Phase 15 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 15 manifest: a required command removed',
    file: 'phase15-manifest',
    expect: /Phase 15 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 15 manifest: the concurrency gate run once instead of three times',
    file: 'phase15-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 15 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 15 manifest: a key the contract does not declare',
    file: 'phase15-manifest',
    expect: /Phase 15 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 15 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-15-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-15-evidence:begin -->\n', '')
        .replace('<!-- phase-15-evidence:end -->\n', ''),
  },
  {
    name: 'phase 15 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 15 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase15Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 15 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 15 evidence was measured at/,
    mutate: (text) =>
      inPhase15Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 14 evidence (check 17)
  {
    name: 'phase 14 manifest: an acceptance the governed state does not record',
    file: 'phase14-manifest',
    expect: /Phase 14 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 14 manifest: a non-zero exit recorded',
    file: 'phase14-manifest',
    expect: /records a non-zero exit code in the Phase 14 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 14 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase14-manifest',
    expect: /Phase 14 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 14 manifest: a required command removed',
    file: 'phase14-manifest',
    expect: /Phase 14 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 14 manifest: the concurrency gate run once instead of three times',
    file: 'phase14-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 14 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 14 manifest: a key the contract does not declare',
    file: 'phase14-manifest',
    expect: /Phase 14 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 14 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-14-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-14-evidence:begin -->\n', '')
        .replace('<!-- phase-14-evidence:end -->\n', ''),
  },
  {
    name: 'phase 14 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 14 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase14Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 14 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 14 evidence was measured at/,
    mutate: (text) =>
      inPhase14Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
  ...
  // ------------------------------------------------ Phase 13 evidence (check 17)
  {
    name: 'phase 13 manifest: an acceptance the governed state does not record',
    file: 'phase13-manifest',
    expect: /Phase 13 manifest declares acceptance = "ACCEPTED"/,
    mutate: (text) =>
      text.replace('"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"', '"acceptance": "ACCEPTED"'),
  },
  {
    name: 'phase 13 manifest: a non-zero exit recorded',
    file: 'phase13-manifest',
    expect: /records a non-zero exit code in the Phase 13 manifest/,
    mutate: (text) => text.replace('"exits": [0]', '"exits": [1]'),
  },
  {
    name: 'phase 13 manifest: measured at the Phase 05 acceptance commit',
    file: 'phase13-manifest',
    expect: /Phase 13 manifest names the Phase 05 acceptance commit as its measured commit/,
    mutate: (text) =>
      text.replace(
        /"measuredAtCommit": "[0-9a-f]{40}"/,
        '"measuredAtCommit": "35314ba210f609269863f0b528bbe827e6a5d3ce"',
  ...
  {
    name: 'phase 13 manifest: a required command removed',
    file: 'phase13-manifest',
    expect: /Phase 13 manifest omits required battery commands: pnpm run test:security/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      manifest.battery = manifest.battery.filter(
        (entry) => entry.command !== 'pnpm run test:security',
  ...
  {
    name: 'phase 13 manifest: the concurrency gate run once instead of three times',
    file: 'phase13-manifest',
    expect:
      /pnpm run test:concurrency must be executed 3 time\(s\); the Phase 13 manifest records 1/,
    mutate: (text) => {
      const manifest = JSON.parse(text);
      const entry = manifest.battery.find((e) => e.command === 'pnpm run test:concurrency');
      entry.executions = 1;
  ...
  {
    name: 'phase 13 manifest: a key the contract does not declare',
    file: 'phase13-manifest',
    expect: /Phase 13 manifest declares keys \[.*remediationNumber/,
    mutate: (text) => text.replace('"battery":', '"remediationNumber": 1,\n  "battery":'),
  },
  {
    name: 'phase 13 evidence: the region markers removed',
    file: 'phase-status',
    expect: /phase-13-evidence: the begin marker text occurs 0 times/,
    mutate: (text) =>
      text
        .replace('<!-- phase-13-evidence:begin -->\n', '')
        .replace('<!-- phase-13-evidence:end -->\n', ''),
  },
  {
    name: 'phase 13 evidence: a result restated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 13 evidence result for pnpm run test:regression is/,
    mutate: (text) =>
      inPhase13Region(text, (region) =>
        region.replace(/(\| `pnpm run test:regression` \| PASS \| )([^|]+)\|/, '$1altered |'),
      ),
  },
  {
    name: 'phase 13 evidence: the measured commit stated differently from the manifest',
    file: 'phase-status',
    expect: /Phase 13 evidence was measured at/,
    mutate: (text) =>
      inPhase13Region(text, (region) =>
        region.replace(
          /Measured at (implementation|correction) commit [0-9a-f]{40}/,
...
for (const fixture of FIXTURES) {
  ...
  try {
    ...
    const paths = {
      ...
      phase11ManifestPath: PHASE11_MANIFEST,
      phase12ManifestPath: PHASE12_MANIFEST,
      phase13ManifestPath: PHASE13_MANIFEST,
      phase14ManifestPath: PHASE14_MANIFEST,
      phase15ManifestPath: PHASE15_MANIFEST,
      phase16ManifestPath: PHASE16_MANIFEST,
    ...
    const KEY = {
      ...
      'phase11-manifest': 'phase11ManifestPath',
      'phase12-manifest': 'phase12ManifestPath',
      'phase13-manifest': 'phase13ManifestPath',
      'phase14-manifest': 'phase14ManifestPath',
      'phase15-manifest': 'phase15ManifestPath',
      'phase16-manifest': 'phase16ManifestPath',
...
for (const [name, path, original] of [
  ...
  ['the Phase 11 manifest', PHASE11_MANIFEST, originalPhase11Manifest],
  ['the Phase 12 manifest', PHASE12_MANIFEST, originalPhase12Manifest],
  ['the Phase 13 manifest', PHASE13_MANIFEST, originalPhase13Manifest],
  ['the Phase 14 manifest', PHASE14_MANIFEST, originalPhase14Manifest],
  ['the Phase 15 manifest', PHASE15_MANIFEST, originalPhase15Manifest],
  ['the Phase 16 manifest', PHASE16_MANIFEST, originalPhase16Manifest],
...
const decoyPhase08Manifest = originalPhase08Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase09Manifest = originalPhase09Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
...
const decoyPhase11Manifest = originalPhase11Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase12Manifest = originalPhase12Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase13Manifest = originalPhase13Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase14Manifest = originalPhase14Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase15Manifest = originalPhase15Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase16Manifest = originalPhase16Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase17Manifest = originalPhase17Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase18Manifest = originalPhase18Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase19Manifest = originalPhase19Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
...
const decoyPhase21Manifest = originalPhase21Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
const decoyPhase22Manifest = originalPhase22Manifest.replace(
  '"acceptance": "AWAITING_CUSTOMER_ACCEPTANCE"',
  '"acceptance": "ACCEPTED"',
);
for (const [label, contents, original] of [
  ...
  ['phase 11 manifest', decoyPhase11Manifest, originalPhase11Manifest],
  ['phase 12 manifest', decoyPhase12Manifest, originalPhase12Manifest],
  ['phase 13 manifest', decoyPhase13Manifest, originalPhase13Manifest],
  ['phase 14 manifest', decoyPhase14Manifest, originalPhase14Manifest],
  ['phase 15 manifest', decoyPhase15Manifest, originalPhase15Manifest],
  ['phase 16 manifest', decoyPhase16Manifest, originalPhase16Manifest],
...
writeFileSync(join(DECOY_DIR, 'phase-11-evidence.json'), decoyPhase11Manifest);
writeFileSync(join(DECOY_DIR, 'phase-12-evidence.json'), decoyPhase12Manifest);
writeFileSync(join(DECOY_DIR, 'phase-13-evidence.json'), decoyPhase13Manifest);
writeFileSync(join(DECOY_DIR, 'phase-14-evidence.json'), decoyPhase14Manifest);
writeFileSync(join(DECOY_DIR, 'phase-15-evidence.json'), decoyPhase15Manifest);
writeFileSync(join(DECOY_DIR, 'phase-16-evidence.json'), decoyPhase16Manifest);
...
for (const [label, paths, expected] of [
  ...
  [
    'phase 08 manifest',
    { phase08ManifestPath: join(DECOY_DIR, 'phase-08-evidence.json') },
    /Phase 08 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 09 manifest',
  ...
  [
    'phase 11 manifest',
    { phase11ManifestPath: join(DECOY_DIR, 'phase-11-evidence.json') },
    /Phase 11 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 12 manifest',
    { phase12ManifestPath: join(DECOY_DIR, 'phase-12-evidence.json') },
    /Phase 12 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 13 manifest',
    { phase13ManifestPath: join(DECOY_DIR, 'phase-13-evidence.json') },
    /Phase 13 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 14 manifest',
    { phase14ManifestPath: join(DECOY_DIR, 'phase-14-evidence.json') },
    /Phase 14 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 15 manifest',
    { phase15ManifestPath: join(DECOY_DIR, 'phase-15-evidence.json') },
    /Phase 15 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 16 manifest',
    { phase16ManifestPath: join(DECOY_DIR, 'phase-16-evidence.json') },
    /Phase 16 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 17 manifest',
    { phase17ManifestPath: join(DECOY_DIR, 'phase-17-evidence.json') },
    /Phase 17 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 18 manifest',
    { phase18ManifestPath: join(DECOY_DIR, 'phase-18-evidence.json') },
    /Phase 18 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 19 manifest',
  ...
  [
    'phase 21 manifest',
    { phase21ManifestPath: join(DECOY_DIR, 'phase-21-evidence.json') },
    /Phase 21 manifest declares acceptance = "ACCEPTED"/,
  ],
  [
    'phase 22 manifest',
...
for (const [name, value] of [
  ...
  ['PRSYSTEM_PHASE11_MANIFEST', join(DECOY_DIR, 'phase-11-evidence.json')],
  ['PRSYSTEM_PHASE12_MANIFEST', join(DECOY_DIR, 'phase-12-evidence.json')],
  ['PRSYSTEM_PHASE13_MANIFEST', join(DECOY_DIR, 'phase-13-evidence.json')],
]) {
