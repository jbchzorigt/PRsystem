...
const FIXTURES = [
  ...
  {
    // The ledger cannot advance a phase the governed state has not: Phase 12 is
    ...
    file: 'phase-status',
    expect: /Phase 12 ledger state cell renders/,
    mutate: (text) =>
      text.replace(
        /^(\| 12 \| Public discovery and Guest authentication \| )`NOT STARTED`/m,
        '$1`DONE`',
      ),
  },
  {
  ...
  },
  {
    ...
    file: 'phase-status',
    expect: /states Current phase = "13 [^"]*"; the governed value is "12 — Public discovery/,
    mutate: (text) =>
      text.replace(
  ...
  },
  {
    ...
    file: 'phase-status',
    expect: /Phase 12 ledger state cell renders "`IN PROGRESS`"/,
    mutate: (text) => text.replace(/^(\| 12 \|[^|]*\| )`NOT STARTED`/m, '$1`IN PROGRESS`'),
  },
  {
