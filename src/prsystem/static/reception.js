...
(() => {
  ...
  async function roomData(seq){const data=await Promise.all([api(path(`rooms?limit=100&after=${enc(roomAfter)}`)),api(path('room-categories')),api(path(`stays/active?limit=100&after=${enc(stayAfter)}`))]);if(seq===generation)[rooms,categories,stays]=data;}
