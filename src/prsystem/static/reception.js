  ...
  async function guestMinibarTasks(container,after){
    ...
    try{
      ...
      for(const t of data.items){
        ...
        const available=item=>t.availability?.[item.product_id]?.available_quantity??item.opening_quantity;
        if(t.availability)table(r,'Баримтжуулсан нөхөлт',['Бараа','Нөхсөн','Нийт боломжит тоо'],t.price_book.items.map(i=>[i.name,t.availability[i.product_id].refill_quantity,available(i)]));
