...
(() => {
  ...
  async function api(url,body,method='POST'){
    const requestToken=token,controller=new AbortController(),timer=setTimeout(()=>controller.abort(),20000);
    try { const response=await fetch(url,{method:body===undefined?'GET':method,headers:{...(token?{Authorization:`Bearer ${token}`} : {}),...(body===undefined?{}:{'Content-Type':'application/json'})},body:body===undefined?undefined:JSON.stringify(body),signal:controller.signal,cache:'no-store'});
      ...
      if(!response.ok){const e=new Error(errors[data.code]||(response.status===422?'Оруулсан талбар, дүн, хугацаагаа шалгана уу.':'Үйлдлийг бүртгэж чадсангүй. Мэдээллээ шинэчлээд дахин шалгана уу.'));e.code=data.code;e.status=response.status;if(response.status===401&&token&&token===requestToken){token='';dirty=false;pending=false;if(location.pathname==='/guest/entry'){$('#login').replaceChildren(node('h1','Нэвтрэх эрх дууссан'),node('p','Өрөөний QR-г дахин уншуулж, Reception-оос шинэ код авна уу.'));}else showLogin('Нэвтрэх эрх дууссан. Дахин нэвтэрнэ үү.');}throw e;}return data;
  ...
  function navigation(){const nav=$('#navigation');nav.replaceChildren();let links=[];if(role('RECEPTION')||manager())links=['guests','rooms','payments',...(packageMnt===30000?['restaurant']:[])];if(role('RECEPTION')||manager()||role('HOTEL_ADMIN'))links.push('shift');if(manager()&&!overview.completion_only)links.push('manager');if(role('CLEANER')||manager())links.push('cleaning');
  ...
  async function render(next,seq){const parent=content();
    if(['guests','rooms','payments','restaurant','manager'].includes(next)){await roomData(seq);if(seq!==generation)return;}
    if(next==='guests'){
      ...
      for(const r of rooms){const card=record(parent,`${r.number} · ${r.category_name}`,`${labels[r.status]} · ${labels[r.cleaning_state]} · ${r.minibar_mode==='OFF'?'Минибаргүй':'Туршилтын минибартай'}`);const a=actions(card);
        ...
        else if(role('RECEPTION')&&!overview.completion_only&&r.status==='ACTIVE')a.append(btn('Зочин бүртгэх',()=>guard(()=>checkin(card,null,r))));
  ...
  if(location.pathname==='/guest/entry')guestEntry();else showLogin();
})();
