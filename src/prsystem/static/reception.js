...
(() => {
  ...
  async function api(url,body,method='POST',access=token){
    ...
    try { const response=await fetch(url,{method:body===undefined?'GET':method,headers:{...(access?{Authorization:`Bearer ${access}`} : {}),...(body===undefined?{}:{'Content-Type':'application/json'})},body:body===undefined?undefined:JSON.stringify(body),signal:controller.signal,cache:'no-store'});
      ...
      if(!response.ok){const e=new Error(errors[data.code]||(response.status===422?'Оруулсан талбар, дүн, хугацаагаа шалгана уу.':'Үйлдлийг бүртгэж чадсангүй. Мэдээллээ шинэчлээд дахин шалгана уу.'));e.code=data.code;e.status=response.status;if(response.status===401&&token&&token===requestToken){token='';dirty=false;pending=false;if(location.pathname==='/guest/entry'){generation++;$('#app').hidden=true;$('#logout').hidden=true;$('#login').hidden=false;content().replaceChildren();$('#login').replaceChildren(node('h1','Нэвтрэх эрх дууссан'),node('p','Өрөөний QR-г дахин уншуулж, Reception-оос шинэ код авна уу.'));}else if(portalMode)portalHome('Нэвтрэх эрх дууссан. Дахин нэвтэрнэ үү.');else showLogin('Нэвтрэх эрх дууссан. Дахин нэвтэрнэ үү.');}throw e;}return data;
  ...
  function form(parent,title,fields,submit,action,opts={}){
    ...
    f.addEventListener('submit',async event=>{event.preventDefault();if(!f.isConnected||busy||pending)return;const values={};let first=null;
      for(const {input,error,spec} of controls.values()){const raw=spec.type==='checkbox'?input.checked:spec.type==='file'?(input.files[0]||''):spec.type==='password'?input.value:input.value.trim();let message='';
        ...
        if(raw!==''&&spec.type==='number'&&(!(spec.decimal?Number.isFinite(Number(raw)):Number.isSafeInteger(Number(raw)))||Number(raw)<(spec.min??0)||Number(raw)>(spec.max??Infinity)))message='Зөв бүхэл дүн оруулна уу.';
  ...
  function showLogin(message=''){if(location.pathname==='/restaurant'){restaurantLogin(message);return;}generation++;$('#app').hidden=true;$('#logout').hidden=true;$('#login').hidden=false;$('#content').replaceChildren();$('#login').replaceChildren();overview=null;roles=[];selected=null;rooms=[];stays=[];categories=[];bookings=[];inventoryAfter='';templateAfter='';templateSelection=null;
  ...
  function portalHome(message=''){
    document.querySelector('header span').textContent=portalMode==='contact'?'Холбоо барих утас':portalMode==='operation'?'Үйл ажиллагааны хяналт':portalMode==='booker'?'Буудлын онлайн захиалга':'Platform санхүү';document.querySelector('header .brand').href=location.pathname;
  ...
  function operationLogin(){portalTitle('Operation нэвтрэх');form(content(),'Platform эрхээр нэвтрэх',[field('email','Имэйл','email',{autocomplete:'username'}),field('password','Нууц үг','password',{autocomplete:'current-password'}),field('code','MFA код','text',{max:6,autocomplete:'one-time-code'})],'Нэвтрэх',v=>api('/platform/auth/login',{email:v.email,password:v.password,code:v.code}),{login:true,success:async r=>{token=r.access_token;portalHome();}});}
  async function operationTools(after=''){
    ...
    try{
      ...
      table(metrics,'Subscription төлөв',['Төлөв','Буудал'],Object.entries(data.statuses).map(([k,v])=>[operationLabels[k],v]));
      ...
      table(parent,'Subscription бүртгэл',['Буудал','Төрөл / хаяг','Утас / имэйл','Багц','Сар','Эхэлсэн','Дуусах','Хоног','Төлөв','Үйлдэл'],data.items.map(i=>[i.name,`${i.owner_kind==='COMPANY'?'Байгууллага':'Хувь хүн'} · ${i.address}`,`${i.phone} · ${i.email}`,money(i.package_mnt),i.months,time(i.starts_at),time(i.expires_at),i.days_left,operationLabels[i.status],'']));
      ...
      say('Subscription мэдээлэл шинэчлэгдлээ.');
  ...
  function operationHotel(hotel){generation++;content().replaceChildren();portalTitle(hotel.name);content().append(node('p',`${hotel.email} · ${hotel.phone} · ${operationLabels[hotel.status]}`));
    ...
    if(operationCan('SUBSCRIPTION_SUSPEND'))form(content(),hotel.suspended?'Subscription сэргээх':'Subscription түдгэлзүүлэх',[reason(),field('reference','Хяналтын кейсийн дугаар')],hotel.suspended?'Сэргээх':'Түдгэлзүүлэх',v=>api(`/platform/hotels/${enc(hotel.tenant_id)}/security/${hotel.suspended?'resume':'suspend'}`,v),{success:()=>operationTools()});
    actions(content()).append(btn('Утас солих дэмжлэг',()=>guard(()=>operationContactSupport(hotel))),btn('Жагсаалт руу',()=>guard(()=>operationTools())));
  }
  function operationSmsComposer(filters){generation++;content().replaceChildren();portalTitle('Subscription сануулах');const panel=node('div');content().append(node('p','Бүртгэлтэй үндсэн утаснуудад үйлчилгээний сануулга илгээнэ. Илгээхээс өмнө хүлээн авагч, текст, сегмент болон зардлыг хянана уу.'));
