...
(() => {
  ...
  let rooms=[],categories=[],stays=[],bookings=[],roomAfter='',stayAfter='',selected=null,inventoryAfter='';
  ...
  function showLogin(message=''){generation++;$('#app').hidden=true;$('#logout').hidden=true;$('#login').hidden=false;$('#content').replaceChildren();$('#login').replaceChildren();overview=null;roles=[];selected=null;rooms=[];stays=[];categories=[];bookings=[];inventoryAfter='';
  ...
  function checkin(parent,booking=null,room=null){
    ...
    form(start,'Бүртгэлийн төрөл',[select('identity_type','Баримтын төрөл',kinds)],'Зочны мэдээлэл оруулах',async v=>v,{success:async(r,f)=>{f.remove();const fields=[field('family_name','Овог'),field('given_name','Нэр'),field('date_of_birth','Төрсөн огноо','date'),field('nationality','Иргэншил')];
      ...
      if(booking?.category_id)fields.push(select('room_id','Оноох өрөө',choices(rooms.filter(r=>r.status==='ACTIVE'),'room_id',r=>`${r.number} · ${r.category_name}`)));
