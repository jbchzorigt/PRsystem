    ...
    def list_rooms(self,bearer,tenant,limit=100,after=''):
        with transaction(self.auth.dsn) as conn:
            ...
            rows=conn.execute('''SELECT r.id,r.number,r.floor,r.category_id,c.name,r.status,r.cleaning_state,r.revision,
                ...
                EXISTS(SELECT 1 FROM prsystem.reception_dependency_blocker b WHERE b.tenant_id=r.tenant_id AND b.room_id=r.id AND b.source_kind='MINIBAR_CONFIGURATION' AND b.state='OPEN')
            ...
            return [dict(room_id=r[0],number=r[1],floor=r[2],category_id=r[3],category_name=r[4],status=r[5],
                         ...
                         cleaning_buffer_minutes=r[17],category_status=r[18],pending_minibar_change=r[20]) for r in rows]
