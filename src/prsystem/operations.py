...
class Operations(GuestFinance):
    def reader(self,conn,bearer,tenant):
        ...
        gate=subscription_gate(Action.DETAIL,AccessFacts(tenant,True,True,True,allowed,True,True,True,hotel[2]),hotel[1],conn.execute('SELECT clock_timestamp()').fetchone()[0])
    ...
    def overview(self,bearer,tenant,after='',limit=50):
        with transaction(self.auth.dsn) as conn:
            principal,hotel,manager,admin=self.reader(conn,bearer,tenant)
