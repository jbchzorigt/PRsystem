...
class OnboardingService:
    ...
    def _proof(conn,row):
        ...
        return (not account or (account[0]==row[8] and account[1]=='ACTIVE' and account[2] is not None)) and (not owner or owner[1]==row[8])
    ...
    def provision(self,app):
        ...
        with transaction(self.auth.dsn) as conn:
            ...
            job=conn.execute('SELECT attempts,next_attempt_at FROM prsystem.onboarding_job WHERE application_id=%s AND completed_at IS NULL FOR UPDATE',(app,)).fetchone()
            if not row[9] or not job:raise DomainError('PAYMENT_REQUIRED')
            now=conn.execute('SELECT clock_timestamp()').fetchone()[0]
            if job[0]>=5 or job[1]>now:raise DomainError('PROVISION_RETRY_BLOCKED')
            if not self._proof(conn,row):
            ...
            conn.execute('UPDATE prsystem.onboarding_job SET completed_at=clock_timestamp(),attempts=attempts+1,last_error_code=NULL WHERE application_id=%s',(app,))
    ...
    def run_job(self,app):
        import psycopg
        try:
            return self.provision(app)
        except psycopg.Error:
            # Main transaction is already rolled back, including deferred COMMIT
            # failures. Persist bounded retry without revealing provider/PII logs.
            with transaction(self.auth.dsn) as conn:
                conn.execute('SELECT id FROM prsystem.onboarding_application WHERE id=%s FOR UPDATE',(app,)).fetchone()
                job=conn.execute('SELECT attempts,completed_at FROM prsystem.onboarding_job WHERE application_id=%s FOR UPDATE',(app,)).fetchone()
                if job and job[1] is None:
                    attempts=job[0]+1
                    conn.execute("UPDATE prsystem.onboarding_job SET attempts=%s,next_attempt_at=clock_timestamp()+%s,last_error_code='DATABASE_FAILURE' WHERE application_id=%s",(attempts,timedelta(seconds=min(3600,30*2**min(attempts,7))),app))
                    conn.execute("UPDATE prsystem.onboarding_application SET state='PROVISIONING_FAILED' WHERE id=%s AND tenant_id IS NULL",(app,))
                    self._event(conn,app,'PROVISIONING_FAILED',dict(attempt=attempts,code='DATABASE_FAILURE'))
    ...
    def manual_retry(self,platform,bearer,app,key,reason,reference):
        ...
        with transaction(self.auth.dsn) as conn:
            ...
            conn.execute('UPDATE prsystem.onboarding_job SET attempts=0,next_attempt_at=clock_timestamp() WHERE application_id=%s AND completed_at IS NULL',(app,))
    ...
    def once(self,limit=25):
        ...
        with transaction(self.auth.dsn) as conn:
            jobs=conn.execute('''SELECT application_id FROM prsystem.onboarding_job
                WHERE completed_at IS NULL AND attempts<5 AND next_attempt_at<=clock_timestamp()
                ORDER BY next_attempt_at,application_id LIMIT %s''',(limit,)).fetchall()
