...
class MinibarAdjustments(MinibarWarehouse):
    ...
    def post(self,conn,tenant,product,data,before,actor,roles,package,*,identity=None,correction_id=None):
        ...
        money(new_total)
        identity=identity or secrets.token_hex(16);receipt=secrets.token_hex(16)
        conn.execute('''INSERT INTO prsystem.minibar_adjustment(tenant_id,id,product_id,room_id,stay_id,kind,quantity,hotel_delta,room_delta,billable_delta,
            original_id,receipt_id,actor_id,actor_roles,package_mnt,reason,correction_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)''',
            (tenant,identity,product,room,stay,kind,quantity,hotel_delta,room_delta,billable_delta,original[0] if original else None,receipt,actor,roles,package,data['reason'],correction_id))
    ...
    def correct(self,bearer,tenant,product,data,key):
        data=dict(data,reason=self._text(data['reason'],1000))
        command=dict(action='MINIBAR_ADJUSTMENT_CORRECTION',product_id=product,data=data)
        with transaction(self.auth.dsn) as conn:
            actor,roles,package=self.actor(conn,bearer,tenant)
            replay=self._receipt(conn,tenant,key,actor,command)
            if replay is not None:return replay
            ...
            before=self.context(conn,tenant,product,data.get('room_id'))
            if (before['stock_revision']!=data['expected_revision'] or before['stay_id']!=data.get('expected_stay_id')
                    or before['physical_quantity']!=data['expected_physical_quantity']):raise DomainError('REVISION_CONFLICT')
            if before['report_locked']:raise DomainError('STOCK_ADJUSTMENT_LOCKED')
            old=conn.execute('SELECT quantity,kind FROM prsystem.minibar_adjustment WHERE tenant_id=%s AND id=%s AND product_id=%s',
                             (tenant,data['original_id'],product)).fetchone()
            if not old:raise DomainError('WORK_SOURCE_NOT_FOUND')
            if old[1]=='REVERSAL' or conn.execute('SELECT 1 FROM prsystem.minibar_adjustment WHERE tenant_id=%s AND original_id=%s',(tenant,data['original_id'])).fetchone():
                raise DomainError('WORK_NOT_OPEN')
            correction,reversal,replacement=(secrets.token_hex(16) for _ in range(3))
            conn.execute('INSERT INTO prsystem.minibar_adjustment_correction(tenant_id,id,original_id,reversal_id,replacement_id,actor_id,reason) VALUES(%s,%s,%s,%s,%s,%s,%s)',
                         (tenant,correction,data['original_id'],reversal,replacement,actor,data['reason']))
            reverse=self.post(conn,tenant,product,dict(data,kind='REVERSAL',quantity=old[0],unit_cost_mnt=None),before,actor,roles,package,identity=reversal,correction_id=correction)
            restored=self.context(conn,tenant,product,before['room_id'])
            corrected=self.post(conn,tenant,product,dict(data,original_id=None),restored,actor,roles,package,identity=replacement,correction_id=correction)
            result=dict(correction_id=correction,original_id=data['original_id'],reversal=reverse,replacement=corrected)
            self.event(conn,tenant,actor,'MINIBAR_ADJUSTMENT_CORRECTED',product,dict(result,reason=data['reason']))
            self._save_receipt(conn,tenant,key,actor,command,result);return result
    ...
    def history(self,bearer,tenant,product,after='',limit=50):
        with transaction(self.auth.dsn) as conn:
            ...
            rows=conn.execute('''SELECT a.id,a.kind,a.quantity,a.room_id,a.stay_id,a.original_id,a.reason,a.actor_id,a.recorded_at,
                r.cost_numerator,r.cost_denominator,EXISTS(SELECT 1 FROM prsystem.minibar_adjustment x WHERE x.tenant_id=a.tenant_id AND x.original_id=a.id),(SELECT number FROM prsystem.room WHERE tenant_id=a.tenant_id AND id=a.room_id),r.actor_label,a.correction_id
                FROM prsystem.minibar_adjustment a JOIN prsystem.minibar_receipt r ON(r.tenant_id,r.id)=(a.tenant_id,a.receipt_id)
                WHERE a.tenant_id=%s AND a.product_id=%s AND a.id>%s ORDER BY a.id LIMIT %s''',(tenant,product,after,limit+1)).fetchall()
            items=[dict(adjustment_id=r[0],kind=r[1],quantity=r[2],room_id=r[3],stay_id=r[4],original_id=r[5],reason=r[6],actor_id=r[7],recorded_at=r[8],cost=dict(numerator=str(r[9]),denominator=str(r[10])),reversed=r[11],room_number=r[12],actor_label=r[13],correction_id=r[14]) for r in rows[:limit]]
