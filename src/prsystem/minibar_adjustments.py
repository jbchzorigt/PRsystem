...
class MinibarAdjustments(MinibarWarehouse):
    def context(self, conn, tenant, product, room=None):
        ...
        return dict(product_id=product,room_id=room,stay_id=stay,stock_revision=revision,physical_quantity=physical,
    ...
    def change(self,bearer,tenant,product,data,key):
        ...
        with transaction(self.auth.dsn) as conn:
            ...
            if before['report_locked']:raise DomainError('MINIBAR_REPORT_LOCKED')
    ...
    def history(self,bearer,tenant,product,after='',limit=50):
        with transaction(self.auth.dsn) as conn:
            ...
            rows=conn.execute('''SELECT a.id,a.kind,a.quantity,a.room_id,a.stay_id,a.original_id,a.reason,a.actor_id,a.recorded_at,
                r.cost_numerator,r.cost_denominator,EXISTS(SELECT 1 FROM prsystem.minibar_adjustment x WHERE x.tenant_id=a.tenant_id AND x.original_id=a.id)
                FROM prsystem.minibar_adjustment a JOIN prsystem.minibar_receipt r ON(r.tenant_id,r.id)=(a.tenant_id,a.receipt_id)
                WHERE a.tenant_id=%s AND a.product_id=%s AND a.id>%s ORDER BY a.id LIMIT %s''',(tenant,product,after,limit+1)).fetchall()
            items=[dict(adjustment_id=r[0],kind=r[1],quantity=r[2],room_id=r[3],stay_id=r[4],original_id=r[5],reason=r[6],actor_id=r[7],recorded_at=r[8],cost=dict(numerator=str(r[9]),denominator=str(r[10])),reversed=r[11]) for r in rows[:limit]]
