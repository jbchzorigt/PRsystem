        ...
        with closing(psycopg.connect(self._dsn, connect_timeout=5)) as conn, conn:
            ...
            if isinstance(command, (ReserveTransfer, SpendCash)):
                ...
                blocked = conn.execute("""SELECT 1 FROM prsystem.cash_drawer d
                    ...
                    LEFT JOIN prsystem.reception_shift s ON(s.tenant_id,s.id)=(d.tenant_id,d.shift_id)
                    WHERE d.tenant_id=%s AND d.id=ANY(%s) AND (w.state<>'OPEN' OR s.state<>'OPEN')""", (ctx.tenant_id,drawer_ids)).fetchone()
                if blocked:
